        ...
        while (AcceptMessage) {
            try {
                ...
                if(msg.contains("/"))
                {
                    ...
                    if (verifyMsg[0].equals("OPERATION") && verifyMsg[1].equals("DISCONNECT"))
                    {
                        errorCallback(ConnectedThread_CHANNEL, MSG_SOCKET_SERVER_DISCONNECT, getDeviceName(),
                                "disconnected, device name = "+getDeviceName() +
                                        ", MAC address = " + getMacAddress() + ", reason = server disconnected");
                        break;
                    }
                }
                // -[20140909, Edwin] 當server主動斷線時，可以再次連線，但ui不會更新的問題
